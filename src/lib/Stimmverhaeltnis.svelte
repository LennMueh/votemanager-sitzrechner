...
<style>
	...
	li { display: grid; grid-template-columns: auto minmax(7rem, 1fr) auto auto auto; align-items: center; gap: .45rem .7rem; padding: .4rem .55rem; border: 1px solid var(--rand); border-radius: var(--radius); }
	...
	@media (max-width: 650px) { .verhaeltnis { grid-template-columns: 1fr; } .torte { width: min(70vw, 18rem); } li { grid-template-columns: auto minmax(7rem, 1fr) auto; } .stimmen, .sitze { grid-column: 2 / -1; } }
</style>
